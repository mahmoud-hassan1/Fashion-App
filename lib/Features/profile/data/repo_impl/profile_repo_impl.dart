...
class ProfileRepoImpl extends ProfileRepo {
  ...
  Future<List<OrderModel>> getMyTodayOrders(DateTime date) async {
    DocumentSnapshot res = await firestoreServices.getDocumentData(
        usersCollectionKey, UserModel.getInstance().uid);
  ...
  @override
  Future<List<SpecificOrderModel>> getMyOrdersOnSpecificDate(
      DateTime date) async {
    QuerySnapshot querySnapshot =
        await firestoreServices.getCollectionData(usersCollectionKey);

    List<SpecificOrderModel> orders = [];

    for (QueryDocumentSnapshot snapshot in querySnapshot.docs) {
      for (int i = 0; i < snapshot.get(OrderModel.ordersKey).length; i++) {
        SpecificOrderModel specificOrderModel = SpecificOrderModel.fromJson(
            snapshot, snapshot.get(OrderModel.ordersKey)[i]);
  ...
  Future<void> saveUserChanges(String name, DateTime dateOfBirth) async {
    ...
    await firestoreServices.updateField(
        usersCollectionKey, userModel.uid, userModel.toMap());
  }
  ...
  Future<List<ProductStatisticsModel>> getProductsBestSelling() async {
    ...
    QuerySnapshot productsQuerySnapshot =
        await firestoreServices.getCollectionData(productsCollectionKey);

    List<String> productsIDs =
        List<String>.generate(productsQuerySnapshot.docs.length, (int index) {
        ...
        for (OrderItemModel item in order.items) {
          if (productsIDs.contains(item.productId)) {
            if (productsQuantities.containsKey(item.productId)) {
              productsQuantities[item.productId] =
    ...
    List<QueryDocumentSnapshot> filteredProductsSnapshot = productsQuerySnapshot
        .docs
        .where((doc) => sortedProductsQuantities.containsKey(doc.id))
        .toList();
    ...
    for (QueryDocumentSnapshot json in filteredProductsSnapshot) {
      ...
      double percentage =
          ((sortedProductsQuantities[productModel.id] ?? 0) / totalQuantity) *
              100;
