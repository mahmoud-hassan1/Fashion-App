...
class MyBagRepoImpl extends MyBagRepo {
  ...
  Future<List<ProductModel>> getMyBagItems() async {
    ...
    if (UserModel.getInstance().bag.isNotEmpty) {
      final QuerySnapshot result = await firestoreServices
          .getCollectionRef(productsCollectionKey)
          .where(FieldPath.documentId, whereIn: UserModel.getInstance().bag)
          .get();
      for (int i = 0; i < result.docs.length; i++) {
        if (result.docs[i]['stock'] > 0) {
          products
              .add(ProductModel.fromJson(result.docs[i], result.docs[i].id));
        }
      }
    }
  ...
  Future<void> checkOut(List<OrderItemModel> items) async {
    if (UserModel.getInstance().bag.isNotEmpty) {
      ...
      DocumentReference doc = firestoreServices.getDocumentRef(
  ...
  Future<void> addToFavourites(String productUID) async {
    if (!UserModel.getInstance().favourites.contains(productUID)) {
      ...
      await favouriteRepo.addToFavourites(
          UserModel.getInstance().uid, productUID);
    }
  }
  ...
  Future<void> removeFromFavourites(String productUID) async {
    if (UserModel.getInstance().favourites.contains(productUID)) {
      ...
      await favouriteRepo.removeFromFavourites(
          UserModel.getInstance().uid, productUID);
    }
  }
  ...
  Future<void> addToBag(String productUID) async {
    DocumentSnapshot productSnapshot =
        await firestoreServices.getDocumentRef('products', productUID).get();
